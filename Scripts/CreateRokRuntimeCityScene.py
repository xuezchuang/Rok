...
NAMED_CIVILIZATION = os.environ.get("ROK_RUNTIME_CITY_NAMED_CIV", "1")
...
SPRITE_PLANE_MODE = os.environ.get("ROK_CITY_SPRITE_PLANE_MODE", "billboard")
...
def import_runtime_texture_as(source_path, asset_name):
...
def spawn_shadow_and_pad(label, primitives, materials, x, y, footprint_width, footprint_length, visual_width):
    shadow_width = max(180.0, visual_width * 0.74)
    shadow_height = max(95.0, footprint_length * 0.72)
    spawn_runtime_plane(
    ...
    spawn_runtime_plane(
        "RuntimeBase_{0}".format(label),
        primitives,
        (x, y, 3.0),
        max(160.0, footprint_width * 1.08),
        max(130.0, footprint_length * 0.95),
        materials.get("M_RuntimeCity_RoadDirt", materials["M_Rok_Path_Blockout"]),
        "RokRuntimeCity/Bases",
        yaw=45.0,
    )
...
def apply_camera_sprite_anchor(actor, height):
    if SPRITE_PLANE_MODE != "camera":
        return
    component = actor.get_component_by_class(unreal.StaticMeshComponent)
    if not component:
...
def spawn_sprite_actor(label, material, x, y, width, height, sort_priority, folder, tags, selectable, footprint=(1, 1), tile=(0, 0), z_override=None):
...
def spawn_target_named_buildings(building_types):
    spawned = []
    skipped = []
    for index, (building_type, display_name, x, y, relative_scale) in enumerate(TARGET_CITY_BUILDING_LAYOUT):
        ...
        if not source_path:
            ...
            continue
        texture = import_runtime_texture_as(source_path, "Cutout_{0}".format(sprite_name))
        if not texture:
            skipped.append("{0}: import failed".format(sprite_name))
            continue
        material = create_runtime_city_sprite_material(texture, "Cutout_{0}".format(sprite_name), None, False, False)
        tex_width, tex_height = texture_dimensions(texture)
...
def spawn_target_resources_and_markers():
    spawned = []
    marker_count = 0
...
def build_runtime_city():
    ...
    if TARGET_SCENE_PASS:
        spawn_target_terrain_layers()
        spawn_roads(primitives, materials)
        spawn_target_wall_anchors(primitives, materials)
        spawn_target_named_buildings(building_types)
        spawn_target_resources_and_markers()
    else:
        spawn_roads(primitives, materials)
